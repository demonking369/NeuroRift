...
class NetworkBridge:
    def _run(self, binary: str, args: list) -> Dict[str, Any]:
        ...
        result = subprocess.run([str(b)] + args, capture_output=True, text=True, timeout=10)
    ...
    def tcp_probe(self, host: str, port: int, timeout_ms: int = 2000) -> Dict[str, Any]:
        return self._run("tcp_probe", ["--target", host, "--port", str(port), "--timeout", str(timeout_ms)])

    def packet_crafter(self, src_ip: str, dst_ip: str, dst_port: int, ttl: int = 64) -> Dict[str, Any]:
        return self._run("packet_crafter", [
            "--src-ip", src_ip, "--dst-ip", dst_ip,
            "--dst-port", str(dst_port), "--ttl", str(ttl)
        ])
