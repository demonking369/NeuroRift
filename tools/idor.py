...
SCHEMA = {
    ...
    "properties": {
        "target": {
            "type": "string",
            "description": "Base URL with an ID parameter, e.g. https://api.example.com/users/1",
        },
        "id_range": {
            "type": "integer",
            "description": "Number of sequential IDs to probe",
            "default": 5,
        },
        "session_cookie": {
            "type": "string",
            "description": "Auth session cookie for context",
            "default": "",
        },
    },
}
...
class IDORTool:
    ...
    def run(
        self, target: str, id_range: int = 5, session_cookie: str = ""
    ) -> ToolResult:
        headers = {}
        cookies = {}
        if session_cookie:
        ...
        for i in range(start_id, start_id + id_range):
            ...
            try:
                resp = requests.get(
                    url, headers=headers, cookies=cookies, timeout=10, verify=False
                )
