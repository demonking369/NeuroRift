...
SCHEMA = {
    ...
    "properties": {
        "command": {
            "type": "string",
            "description": "Shell command to run (must use a whitelisted tool)",
        },
        "timeout": {
            "type": "integer",
...
class ShellExecTool:
    name = "shell_exec"
    description = (
        "Execute external security tools from the whitelist in a sandboxed manner"
    )
    ...
    def run(self, command: str, timeout: int = 60) -> ToolResult:
        ...
        if not parts:
            return ToolResult(
                tool_name=self.name, target="", success=False, error="Empty command"
            )
