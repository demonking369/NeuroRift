...
SCHEMA = {
    ...
    "properties": {
        "target": {
            "type": "string",
            "description": "URL endpoint to test for race conditions",
        },
        "concurrency": {
            "type": "integer",
...
class RaceConditionTool:
    ...
    def run(
        self, target: str, concurrency: int = 20, method: str = "POST"
    ) -> ToolResult:
        try:
            codes = asyncio.run(_race(target, concurrency, method))
        except Exception as e:
            return ToolResult(
                tool_name=self.name, target=target, success=False, error=str(e)
            )
